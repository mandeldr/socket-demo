...
Two things make it more than a loop over the file. A package can be installed
more than once at different versions, so a lock path rather than a name
identifies a node; and an entry can be installed under a name that is not its
own, so the name has to come from the entry rather than from where it sits.
"""
...
def _installed(entries: dict) -> DependencyGraph:
    ...
    queue: deque[tuple[str, int, PackageKey | None]] = deque(
        (path, 0, None) for path in _paths_required_by("", entries, packages, PROJECT_FIELDS)
    )
    ...
    while queue:
        ...
        for target in _paths_required_by(path, entries, packages, INSTALLED_PACKAGE_FIELDS):
